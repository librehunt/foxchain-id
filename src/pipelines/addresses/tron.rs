...
mod tests {
    ...
    #[test]
    fn test_tron_pipeline_compressed_key() {
        let compressed_key =
            hex::decode("0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798")
                .unwrap();
        let params = json!({});

        let result = execute_tron_pipeline(&compressed_key, &params);
        assert!(result.is_ok());
    ...
    #[test]
    fn test_tron_pipeline_uncompressed_key() {
        ...
        let result = execute_tron_pipeline(&uncompressed_key, &params);
        assert!(result.is_ok());
        let address = result.unwrap();
        assert!(!address.is_empty());
    }
    ...
    #[test]
    fn test_tron_pipeline_64_byte_key() {
        ...
        let result = execute_tron_pipeline(&key_64, &params);
        assert!(result.is_ok());
        let address = result.unwrap();
        assert!(!address.is_empty());
    }
    ...
    #[test]
    fn test_tron_pipeline_invalid_length() {
        let invalid_key = vec![0u8; 32];
        let params = json!({});

        let result = execute_tron_pipeline(&invalid_key, &params);
        assert!(result.is_err());
    }

    #[test]
    fn test_tron_pipeline_invalid_decompressed_format() {
